...
// Публічний тестовий API. POST/PATCH/DELETE нічого реально не зберігають
// (завжди повертають "успішний", але фейковий результат) - заміни BASE_URL
// на свій бекенд, коли він буде.
...
async function addTodoApi(payload) {
  const res = await fetch(BASE_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  if (!res.ok) throw new Error(`Помилка додавання: ${res.status}`);
  return res.json();
}

async function updateTodoApi({ id, ...changes }) {
